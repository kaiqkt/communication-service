...
class SendSmsPathMock(private val holder: MockServerHolder) {
    ...
    fun mockSendSms(accountSid: String) {
        mockServer.`when`(
            HttpRequest.request()
                ...
                .withPath("/Accounts/$accountSid/Messages.json")
        ).respond(
    ...
    fun mockSendSmsError() {
        mockServer.`when`(
            HttpRequest.request()
                .withMethod("POST")
                .withHeader(Headers.CONTENT_TYPE, MediaType.APPLICATION_FORM_URLENCODED_VALUE)
                .withPath("/Accounts/${ULID.random()}/Messages.json")
        ).respond(
            HttpResponse.response()
                .withStatusCode(404)
        )
    ...
    fun verifySendSms(accountSid: String, count: Int) = verify("/Accounts/$accountSid/Messages.json", count)
